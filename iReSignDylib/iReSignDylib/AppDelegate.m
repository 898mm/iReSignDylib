...
static NSString *kBookName                          = @"newyork";
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    self.window.title = @"iReSignDylibQH 4.0";
...
- (void)doCodeSigning {
    ...
    for (NSString *file in dirContents) {
        if ([[[file pathExtension] lowercaseString] isEqualToString:@"app"]) {
            ...
            NSLog(@"Found appPath :%@",appPath);
            appName = file;
            if ([[NSFileManager defaultManager] fileExistsAtPath:plugInsDirPath]) {
                NSLog(@"Found plugInsDirPath :%@",plugInsDirPath);
                hasFrameworks = YES;
                NSArray *plugInsContents = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:plugInsDirPath error:nil];
                for (NSString *plugInFile in plugInsContents) {
                    ...
                    if ([extension isEqualTo:@"appex"] ) {
                        plugInPath = [plugInsDirPath stringByAppendingPathComponent:plugInFile];
                        NSLog(@"Found plugInPath :%@",plugInPath);
                        [frameworks addObject:plugInPath];
                    }
                }
            }
            break;
...
- (void)doZip {
    if (appPath) {
        ...
        fileName = [fileName stringByAppendingString:[self getCurrentTimes]];
...
- (void)disableControls {
    ...
    [dylibBrowseButton setEnabled:FALSE];
...
- (void)enableControls {
    ...
    [dylibBrowseButton setEnabled:TRUE];
...
- (void)getCerts {
    ...
    //    [certTask setArguments:[NSArray arrayWithObjects:@"find-certificate", @"-c", @"iPhone Distribution: Great Wall Glory Securities Co., Ltd.", nil]];
    
    //    8) 508F25BC8573BD0FD41E860DA268BDDAC178097F "iPhone Distribution: Great Wall Glory Securities Co., Ltd."
    
    
    //keychain: "/Users/mapboo/Library/Keychains/login.keychain-db"
    //export -k newcert.keychain -t identities -f pkcs12 -o /tmp/mycerts.p12
    
    //   [certTask setArguments:[NSArray arrayWithObjects:@"export", @"-k", @"/Users/mapboo/Library/Keychains/login.keychain-db", @"-t", @"identities", @"-f",@"pkcs12",@"-P",@"123456", @"-o",@"/Users/mapboo/Desktop/DBBackup/ppPP.p12",nil]];
    
    //    [certTask setArguments:[NSArray arrayWithObjects:@"dump-keychain",nil]];
...
-(void)doDylibCopy{
    if ([[NSFileManager defaultManager] fileExistsAtPath:[workingPath stringByAppendingPathComponent:kPayloadDirName]]) {
        ...
        NSString *dylibPath = [dylibPathField stringValue];//[[NSBundle mainBundle] pathForResource:kLibSignatureCheckDylib ofType:nil];
...
-(void)doInjection{
    ...
    for (NSString *fileName in fileDirArray) {
        ...
        if ([fileName pathExtension].length == 0) {
            ...
            if (!isDirectory) {
                ...
                //                long long size = [dic[@"NSFileSize"] longLongValue]/(1024.0*1024.0);
                //                NSLog(@"subPath:%@==dic==%@==%lld",subPath,dic[@"NSFileSize"],size);
            }
        }
    }
    ...
    //    NSString *valueName = [valuePaht lastPathComponent];
